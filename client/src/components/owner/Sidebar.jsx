  ...
  return (
    <div className="bg-gradient-to-r from-[#fffbee] to-white">
      <div className="mx-auto max-w-[1440px] flex flex-col md:flex-row">
        {/* sidebar */}
        <div
          className="max-md:flexCenter flex flex-col justify-between bg-white sm:m-3 md:min-w-[20%] md:min-h-[97vh]
        rounded-xl shadow"
        >
            ...
            <div className="flex md:flex-col md:gap-x-5 gap-y-8 md:mt-4">
              {navItems.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    isActive
                      ? "flexStart gap-x-2 p-5 lg:pl-12 bold-13 sm:!text-sm cursor-pointer h-10 bg-secondary/10 max-md:border-r-4 border-secondary"
